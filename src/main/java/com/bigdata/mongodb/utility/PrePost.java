package com.bigdata.mongodb.utility;

import com.bigdata.mongodb.utility.MemoryLogger;
...
//	BufferedWriter writer = null;
	public Map<List<Integer>, Integer> listItemWithSupport = new HashMap<>();
...
//	 *            the output file path
	 ...
	 */
	public void runAlgorithm(List<List<Integer>> listTransaction, double minsup)
			throws IOException {
		outputCount = 0;
		nlNodeCount = 0;
...
//		writer = new BufferedWriter(new FileWriter(output));
		...
		// ==========================
		// Read Dataset
		getData(listTransaction, minsup);

		resultLen = 0;
		result = new int[numOfFItem];

		// Build tree
		buildTree(listTransaction);
...
//	 */
	void buildTree(List<List<Integer>> listTransaction) throws IOException {
		...
		Iterator<List<Integer>> iteratorTransaction = listTransaction.iterator();
		// for each line (transaction) until the end of the file
		while (iteratorTransaction.hasNext()) {



			List<Integer> transactionLine = iteratorTransaction.next();
			// for each item in the transaction
			int tLen = 0; // tLen
			for (Integer tran : transactionLine) {
				// get the item
				int itemX = tran;
...
//	}
	...
	void getData(List<List<Integer>> listTransaction, double support) throws IOException {
		numOfTrans = 0;
		...
		while (iteratorTransaction.hasNext()) {
			...
			List<Integer> transaction = iteratorTransaction.next();
			// for each item in the transaction
...
//		StringBuilder buffer = new StringBuilder();
		...
		if(curNode.support >= minSupport) {
			...
			for (int i = 0; i < resultLen; i++) {
				listItem.add(item[result[i]].index);
//				buffer.append(item[result[i]].index);
//				buffer.append(' ');
			}
			// append the support of the itemset
			listItemWithSupport.put(listItem, curNode.support);
//			buffer.append("#SUP: ");
		...
		if (sameCount > 0) {
			...
			for (long i = 1, max = 1 << sameCount; i < max; i++) {
				for (int k = 0; k < resultLen; k++) {
					listItem.add(item[result[k]].index);
...
//					buffer.append(' ');
				...
				for (int j = 0; j < sameCount; j++) {
					...
					int isSet = (int) i & (1 << j);
					if (isSet > 0) {
						listItem.add(item[sameItems[j]].index);
